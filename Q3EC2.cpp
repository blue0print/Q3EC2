...
int main() {
    ...
    int xRes=640*1.5, yRes=480*1.5;
    int pos=0, n=0;
    double cReal=0, cImag=0, delta=0.0001;
    int val=0;
    string filename="fractal";

    PGMImage *img = new PGMImage(xRes,yRes,filename);

    double cxmin = -0.75, cymin = 0.1, cxmax = cxmin + delta, cymax = cymin + delta;

    for (int zeta=0; zeta<30; zeta++) {
        ...
        filename += to_string(zeta+1);
        img->writeFile(filename);
        //cout << filename << endl; // *TEST*
        // Modify filename to automatically generate images which will be used for animation
        ...
        filename.erase(pos,n);
        // Shift the envelope by "delta"
        cxmin+=delta;
        cymin-=delta;
        cxmax-=delta;
        cymax+=delta;
